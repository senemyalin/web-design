...
<div class="container" style="max-width: 100%; width: 100%">
    ...
    <nav>
        ...
        ?>
        <form style="margin-left: 50px; display: inline;" method="GET" action="hardbooks.php">
            <input name="s" style="border-radius: 5px;" class="custom-control-input" placeholder="Find a hardbook" formaction="php/logout.php">
...
<div>
    <p id="p2">
        ...
        if(isset($_GET['s'])){
            $s=$_GET['s'];
            $items = $connection->query("SELECT item_id, item_name, item_price, description FROM items WHERE item_category='hardbooks' AND item_name LIKE '%$s%';");
        }
        else
            $items = $connection->query("SELECT item_id, item_name, item_price, description FROM items WHERE item_category='hardbooks';");
        if ($items->rowCount() == 0)
            echo "NO BOOKS";
        else {
